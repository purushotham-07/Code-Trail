...
const Pagination = memo(function Pagination({ page, totalPages, onPageChange }) {
  ...
  return (
    <nav className="flex items-center justify-center gap-2" aria-label="Pagination">
      ...
      {start > 1 && <span className="px-1 text-slate-600">…</span>}
      {pages.map((p) => (
        <motion.button
          key={p}
          whileTap={{ scale: 0.96 }}
          ...
          className={`rounded-md px-3 py-1.5 text-sm transition-colors ${
            p === page ? 'bg-blue-600 text-white' : 'border border-slate-700 text-slate-300 hover:bg-slate-800'
          }`}
        >
          {p}
        </motion.button>
      ))}
      {end < totalPages && <span className="px-1 text-slate-600">…</span>}
      <motion.button
        whileTap={{ scale: 0.96 }}
        type="button"
