...
export function SnippetCardSkeleton() {
  return (
    <motion.div {...pulse} className="rounded-xl border border-slate-800 bg-slate-900 p-5">
...
export function CodeEditorSkeleton() {
  return (
    <motion.div {...pulse} className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      ...
      {Array.from({ length: 12 }).map((_, index) => (
        <div key={index} className="mb-2 h-3 rounded bg-slate-800" style={{ width: `${90 - (index % 4) * 12}%` }} />
      ))}
    </motion.div>
  );
}